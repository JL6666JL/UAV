...
def get_all_score():
    images_folder="/root/autodl-tmp/ans_test/images"

    ans_labels_folder="/root/autodl-tmp/ans_test/labels"
    my_result_folder="./yolov8s_result"
    ...
    all_epoch=[50,100,150,200,250,300,350,400,450,500,550]

   
    #遍历所有结果
    for epoch in all_epoch:
        one_result_path=my_result_folder+"/predict"+str(epoch)+"/labels"
        #print(one_result_path)
        ...
        for i,image_file_name in enumerate(image_file_list):
            ...
            if(vt==1 and pt==0):
                with open(given_result_path,'r') as file:
                    given_content=file.readlines()[0].split(' ')
                #print(given_content)
                ...
                with open(my_result_path,'r') as file:
                    my_content=file.readlines()[0].split(' ')
                #print(my_content)
        ...
        print((epoch,one_score))
        all_score.append(one_score)
    plt.plot(all_epoch, all_score)
    plt.xlabel('epoch')
